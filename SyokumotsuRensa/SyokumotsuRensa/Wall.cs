    ...
    class Wall
    {
        ...
        public void Draw(Renderer renderer)
        {
            for (int i = 0; i < masu; i++)
            {
                ...
                {
                    renderer.DrawTexture("horizontalFence", new Vector2(i * TextureSize + position.X, position.Y));
                }
                else if (direction == Direction.BOTTOM)
                {
                    renderer.DrawTexture("verticalFence", new Vector2(position.X, i * TextureSize + position.Y));
                }
            }
        }
    }
